...
STR_TABLE=$STR_ANALYSIS_DIR/CompareSTRDatabases/PathogenicLoci_GRCh37_20191023_table_Simple_EHDN_Version_test.tsv
...
#	if [ ! -f ${Gene}_${Size}_Het_EHDN.str_profile.json ]; then
	...
	$EHDN8 profile \
		...
		--output-prefix ${Gene}_${Size}_Het_EHDN8 

	$EHDN62 --bam $OUTPUT_BAM \
		--reference $GENOME_FASTA \
		--output ${Gene}_${Size}_Het_EHDN62.json
